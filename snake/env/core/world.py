...
class World(object):
    def __init__(self, size, custom, start_position, start_direction_index, food_position):
        ...
        self.size = size
        self.world = np.zeros([self.size[0], self.size[1]])
    ...
    def init_snake(self):
        ...
        if not self.custom:
            ...
            nums = []
            nums.append(random.randint(1 + SNAKE_SIZE, self.size[0] - 1 - SNAKE_SIZE))
            nums.append(random.randint(1 + SNAKE_SIZE, self.size[1] - 1 - SNAKE_SIZE))
            start_position = tuple(nums)
    ...
    def init_food(self):
        ...
        self.current_available_food_positions = set(zip(*np.where(self.world != self.WALL)))

        for i in snake.blocks:
            self.current_available_food_positions.remove(i)
    ...
    def move_snake(self, action):
        ...
        if self.snake.alive:
            ...
            # Code if snake is outside bounds
            is_outside = False
            wall_bounds = set(zip(*np.where(self.world == self.WALL)))
            for i in wall_bounds:
                if new_snake_head == i:
                    is_outside = True

            # Code if snake eats itself
            in_snake = False
            for i in range(1, len(self.snake.blocks)):
                if new_snake_head == self.snake.blocks[i]:
                    in_snake = True
